...
const EditStoryRequestForm: React.FC<EditStoryRequestFormProps> = ({ onSubmit }) => {
	...
	const handleChange = (value: any) => {
		...
		if (inputValue !== '') {
			setstate((prevState) => ({ ...prevState, buttonEnabled: true, emailValue: inputValue }))
		} else {
			setstate((prevState) => ({ ...prevState, buttonEnabled: false, emailValue: inputValue }))
		}
	}
	...
	const handleSubmit = (e: any) => {
		...
		if (inputValue !== '') {
			if (checkEmail(inputValue)) {
				...
				// if email exists, proceed...else, show error
				onSubmit(1)
				window.scrollTo(0, 0)
			} else {
