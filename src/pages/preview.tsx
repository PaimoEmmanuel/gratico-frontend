...
const Preview: React.FC = () => {
	...
	const [uploading, setUploading] = useState({ status: false, note: '' })

	const handleUpload = (e: React.MouseEvent<HTMLButtonElement>) => {
		// let button = e.target as HTMLInputElement;
		setUploading({ status: true, note: '' })
		postStory({
			name,
			email,
			title,
			...
			.then((res) => {
				if (res.data.uuid) {
					...
					setUploading({
						status: false,
						note: 'An error occured, please try again.',
					})
				}
			})
			.catch((err) => {
			...
			<Content>
				...
				<Story>
					...
					<Button onClick={handleUpload}>
						{uploading.status ? 'Uploading story' : 'Upload story'}
					</Button>
					{uploading.note && (
						<Text style={{ color: 'red', textAlign: 'center' }}> {uploading.note}</Text>
					)}
					<Back
						onClick={(e) => {
							e.preventDefault()
